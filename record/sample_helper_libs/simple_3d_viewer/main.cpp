...
std::ofstream jointsDataFile;
bool isRecording = false;

void InitializeJointsDataFile() {
    if (!jointsDataFile.is_open()) {
        jointsDataFile.open("jointsData.csv", std::ios::out);
        jointsDataFile << "BodyID,JointIndex,PosX,PosY,PosZ,ConfidenceLevel\n";
        if (!jointsDataFile) {
...
void CloseJointsDataFile() {
    if (jointsDataFile.is_open()) {
        jointsDataFile.close();
        std::cout << "Recording stopped and data saved." << std::endl;

        // run python script
        //const char* command = "python C:\\Users\\xxt23\\Desktop\\Azure-Kinect-Samples-master\\body-tracking-samples\\simple_3d_viewer\\build\\bin\\Debug\\convertdata.py";
        const char* command = "python \"C:\\Users\\Venkatasai Gudisa\\Desktop\\hexd\\record\\simple_3d_viewer\\build\\bin\\Debug\\convertdata.py\"";
...
int64_t ProcessKey(void* /*context*/, int key)
{
    ...
    case GLFW_KEY_H:
        PrintAppUsage();
        break;
    case GLFW_KEY_S:  // start
        isRecording = true;
        InitializeJointsDataFile();
...
// save data func
void SaveJointDataToFile(uint32_t bodyId, int jointIndex, const k4a_float3_t& position, k4abt_joint_confidence_level_t confidence) {
    if (isRecording) {  // Only when recording
        jointsDataFile << bodyId << "," << jointIndex << ","
            << std::fixed << std::setprecision(3) << position.xyz.x << ","
            << std::fixed << std::setprecision(3) << position.xyz.y << ","
            << std::fixed << std::setprecision(3) << position.xyz.z << ","
            << confidence << "\n";
    }
}
    ...
    for (uint32_t i = 0; i < numBodies; i++)
    {
        ...
        for (int joint = 0; joint < static_cast<int>(K4ABT_JOINT_COUNT); joint++)
        {
            if (body.skeleton.joints[joint].confidence_level >= K4ABT_JOINT_CONFIDENCE_LOW)
            {
                ...
                // save joint data
                SaveJointDataToFile(body.id, joint, jointPosition, confidence);
